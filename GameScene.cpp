...
GameScene::GameScene(AssetStore* in_assets, SceneManager* in_sceneManager, sf::Vector2u const in_windowSize) : Scene(in_assets, in_sceneManager)
{
	...
	//setup input handler
	std::vector<sf::View> views = { m_gameView };
	m_inputHandler = std::make_unique<GameInputHandler>(GameInputHandler(this, in_sceneManager, views));
...
void GameScene::ZoomView(sf::RenderWindow& in_window, bool in_isZoomingIn)
{
	...
	m_viewScroll = in_isZoomingIn ? m_viewScroll + m_VIEW_SCROLL_INCREMENT : m_viewScroll - m_VIEW_SCROLL_INCREMENT;
...
void GameScene::PanView(sf::RenderWindow& in_window, sf::Vector2i in_mousePosInWindow) 
{
	...
	if (in_window.getViewport(m_gameView).contains(in_mousePosInWindow))
	{
		...
		if (in_mousePosInWindow.x > in_window.getSize().x - m_VIEW_MOVE_BOUNDS) moveDir.x += 1;
		if (in_mousePosInWindow.y > in_window.getSize().y - m_VIEW_MOVE_BOUNDS) moveDir.y += 1;
